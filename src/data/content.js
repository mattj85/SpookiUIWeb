...
export const FEATURES = [
  {
    ...
    body:
      'Edit a value and SpookiUI writes the config and triggers a Ghostty reload for you — the very terminal you are in repaints instantly. No manual reload, no restart.',
  },
  {
    icon: '🛡️',
    title: 'Safe by construction',
    body:
      'Every change is validated by Ghostty itself (ghostty +validate-config) before it is saved. Invalid values are rejected and rolled back, so a bad edit never reaches your config.',
  },
  {
    icon: '↩️',
    title: 'Always reversible',
    body:
      'A dated backup is written on the first change of the day. Revert an entire session with R, or wipe back to Ghostty’s built-in defaults with X — a backup is still kept.',
  },
  {
    icon: '🔍',
    title: 'Every option, discovered dynamically',
    body:
      'Options are never hard-coded. They are read from your installed Ghostty at startup, so the tool always matches your version. Settings for the other OS are hidden automatically.',
  },
  {
    icon: '🎨',
    title: 'Typed editors with live preview',
    body:
      'Themes show a live colour card, fonts and enums open searchable pickers, bounded numbers get visual sliders, and keybindings use a guided builder — all previewed before you commit.',
  },
  {
    icon: '🗂️',
    title: 'Profiles & a config doctor',
    body:
      'Save named snapshots of your whole config and flip between light and dark instantly. The built-in doctor health-checks for duplicates, unknown keys and keybind clashes.',
  },
  {
    icon: '👻',
    title: 'Treats — background shaders',
    body:
      'A bundle of nostalgic animated background shaders (Matrix Rain, Plasma, Fireworks, seasonal packs…). All off by default, one at a time, and tuned to keep your text legible.',
  },
  {
    icon: '⌨️',
    title: 'Fully scriptable CLI',
    body:
      'Everything the UI does is available non-interactively: get, set, list, doctor, profiles, treats and more — perfect for dotfiles and pre-commit hooks.',
  },
]
...
export const LIVE_LOOP = [
  ...
  { step: 'SpookiUI writes the config file', detail: 'surgically, preserving your comments and layout' },
  { step: 'Ghostty validates it', detail: 'an invalid value is rejected & rolled back' },
  { step: 'Ghostty reloads', detail: 'macOS: Reload Configuration menu · Linux: SIGUSR2' },
  { step: 'Your terminal repaints', detail: 'the change is live, in front of you' },
]
...
export const CLI_EXAMPLES = [
  { cmd: './spookiui.py set theme "Catppuccin Latte"', note: 'Change a value — writes + reloads live' },
  { cmd: './spookiui.py set font-size 15', note: 'Numbers, colours, enums — all typed' },
  { cmd: './spookiui.py get background-opacity', note: 'Read an option’s current value' },
  { cmd: './spookiui.py doctor', note: 'Health-check the config; exits non-zero on errors' },
  { cmd: './spookiui.py profile toggle', note: 'Flip between your light and dark profiles' },
  { cmd: './spookiui.py treats enable matrix-rain', note: 'Turn on a background shader' },
]
...
export const SEASONAL_TREATS = [
  { name: 'Ghosts', season: 'Halloween', desc: 'Friendly cartoon ghosts drift by' },
  { name: 'Spooky Eyes', season: 'Halloween', desc: 'Pairs of glowing eyes blink open in the dark' },
  { name: 'Cobwebs', season: 'Halloween', desc: 'Faint spider webs shimmer across the corners' },
  { name: 'Snow', season: 'Winter', desc: 'Soft snowflakes drift down across depth layers' },
]
